...
class HighThroughputExecutor(StatusHandlingExecutor, RepresentationMixin):
    ...
    def initialize_scaling(self):
        ...
        max_workers = (
            ...
            else "--max_workers={}".format(self.max_workers)
        )
    ...
    def start(self, results_passthrough=None):
        ...
        if self.interchange_local is True:
            ...
            logger.info(
                f"Started local interchange with ports: {self.worker_task_port}. {self.worker_result_port}"
            )

        logger.debug(
            "Created management thread: {}".format(self._queue_management_thread)
        )
                ...
                if msgs is None:
                    ...
                    for serialized_msg in msgs:
                        ...
                        if tid == -1 and "exception" in msg:
                            ...
                            logger.warning(
                                "[MTHREAD] Executor shutting down due to fatal exception from interchange"
                            )
                        ...
                        if self.passthrough is True:
                            ...
                            logger.debug(
                                f"[MTHREAD] task:{tid} items in queue: {self.results_passthrough.qsize()}"
                            )
                            continue
                        ...
                        if "result" in msg:
                            ...
                            raise BadMessage(
                                "[MTHREAD] Message received is neither result or exception"
                            )
...
def executor_starter(htex, logdir, endpoint_id, logging_level=logging.DEBUG):
    ...
    with daemon.DaemonContext(stdout=stdout, stderr=stderr):
        ...
        logger = set_file_logger(
            os.path.join(logdir, "executor.{}.log".format(endpoint_id)),
            level=logging_level,
        )
        htex.start()
