...
class BadRegistration(Exception):
    ...
    def __repr__(self):
        return "Manager:{} caused a {} failure".format(self.worker_id, self.handled)
...
class Interchange(object):
    ...
    def __init__(
        ...
        logger.info(
            "Initializing Interchange process with Endpoint ID: {}".format(endpoint_id)
        )
        ...
        logger.info(
            "Task incoming on tcp://{}:{}".format(client_address, client_ports[0])
        )
        self.task_incoming.connect(
            "tcp://{}:{}".format(client_address, client_ports[0])
        )

        self.results_outgoing = self.context.socket(zmq.DEALER)
        self.results_outgoing.set_hwm(0)
        logger.info(
            "Results outgoing on tcp://{}:{}".format(client_address, client_ports[1])
        )
        self.results_outgoing.connect(
            "tcp://{}:{}".format(client_address, client_ports[1])
        )
        ...
        logger.info(
            "Command channel on tcp://{}:{}".format(client_address, client_ports[2])
        )
        self.command_channel.connect(
            "tcp://{}:{}".format(client_address, client_ports[2])
        )
        ...
        if container_uuid not in self.containers:
            if container_uuid == "RAW" or not container_uuid:
                ...
                try:
                    ...
                    logger.info(
                        "[FETCH_CONTAINER] Got container info: {}".format(container)
                    )
    ...
    def _hold_block(self, block_id):
        ...
        for manager in self._ready_manager_queue:
            if (
                ...
                logger.debug(
                    "[HOLD_BLOCK]: Sending hold to manager: {}".format(manager)
                )
            ...
            if (
                ...
                if manager not in self._ready_manager_queue:
                    ...
                    try:
                        ...
                        logger.warning(
                            "[MAIN] Got a non-json registration message from manager:{}".format(
                                manager
                            )
                        )
                    ...
                    if message[1] == b"HEARTBEAT":
                        logger.debug(
                            "[MAIN] Manager {} sends heartbeat".format(manager)
                        )
                ...
                if manager not in self._ready_manager_queue:
                    ...
                    try:
                        ...
                        if manager_report.task_statuses:
                            logger.info(
                                f"[MAIN] Got manager status report: {manager_report.task_statuses}"
                            )
                        ...
                        logger.info(
                            f"[MAIN] Got container switch count: {self.container_switch_count}"
                        )
                    except Exception:
                        pass
                    if len(b_messages):
    ...
    def scale_out(self, blocks=1, task_type=None):
        ...
        for _i in range(blocks):
            if self.provider:
                ...
                logger.debug(
                    "Launched block {}->{}".format(external_block_id, internal_block)
                )
    ...
    def scale_in(self, blocks=None, block_ids=None, task_type=None):
        ...
        if task_type:
            logger.info(
                "Scaling in blocks of specific task type {}. Let the provider decide which to kill".format(
                    task_type
                )
            )
            if self.scaling_enabled and self.provider:
                ...
                logger.info(
                    "Get the killed blocks: {}, and status: {}".format(to_kill, r)
                )
                for job in to_kill:
                    logger.info(
