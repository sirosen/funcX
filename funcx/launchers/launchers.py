...
cat << APRUN_EOF > redis_cmd_$JOBNAME.sh
hostname -i | awk '{{print $1}}' >> $PWD/hostname.txt
{redis_server_command}
APRUN_EOF
chmod a+x redis_cmd_$JOBNAME.sh
rm -f $PWD/hostname.txt
aprun -n {redis_nodes} -N 1 {overrides} /bin/bash redis_cmd_$JOBNAME.sh &
echo "Creating redis cluster"
echo "================================================================="
create-redis -r {redis_replica} -p {redis_port} -n {redis_nodes} -a $PWD/hostname.txt
eval "$(create-redis -r {redis_replica} -p {redis_port} -n {redis_nodes} -a $PWD/hostname.txt)"
cat << APRUN_EOF > cmd_$JOBNAME.sh
export REDIS_SERVERS="{redis_port};`cat $PWD/hostname.txt | tr '\\n' ';'`"
hostname -i | awk '{{print $1}}' >> $PWD/tmp.out
{command}
APRUN_EOF
chmod a+x cmd_$JOBNAME.sh
echo "Starting funcx managers"
echo "==================================================================="
aprun -n {tasks_per_block} -N {tasks_per_node} {overrides} /bin/bash cmd_$JOBNAME.sh &
wait
[[ "{debug}" == "1" ]] && echo "Done"
'''.format(command=command,
           redis_server_command=redis['command'],
...
if __name__ == '__main__':
    ...
    s = AprunLauncher()
    wrapped = s("hello", 1, 1, {'replica': 1, 'nodes': 1, "port": 7000, 'command': 'hello'})
    print(wrapped)
