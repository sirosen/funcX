...
from funcx.executors.workqueue import exec_parsl_function
...
a
    ...
    def __init__(self,
        ...
        if self.config.working_dir is None:
            self.working_dir = "{}/{}".format(self.logdir, "worker_logs")
        logger.info("Setting working_dir: {}".format(self.working_dir))
        ...
        self.address = interchange_address
        self.port = port
        self.task_counter = -1
    ...
    def start(self):
        ...
        self.wq_log_dir = os.path.join(self.run_dir, 'workqueue_log')
        logger.debug("function data directory: {}\nlog directory: {}".format(self.function_data_dir, self.wq_log_dir))
        os.makedirs(self.function_data_dir, exist_ok=True)
        os.makedirs(self.package_dir, exist_ok=True)
        os.makedirs(self.wq_log_dir, exist_ok=True)
        ...
        self._migrate_tasks_to_internal(self._kill_event, self._status_request)
    ...
    def _migrate_tasks_to_internal(self, kill_event, status_request):
        ...
        while not kill_event.is_set():
            ...
            try:
                ...
                #logger.debug("[TASK_PULL_THREAD] {} tasks in internal queue".format(task_counter))
                continue
            ...
            if msg == 'STOP':
                ...
                logger.debug("[TASK_PULL_THREAD] pending task count: {}".format(task_counter))
                ...
                os.makedirs(self._path_in_task(task_id), exist_ok=True)
...
#                     self.tasks[str(task_id)] = fu
                ...
                logger.debug("Creating task {} for msg {}".format(task_id, msg))
                ...
                category = msg['task_id'].split(';')[1] if self.autocategory else 'parsl-default'
                logger.debug("Tagging task {} as category {}".format(msg['task_id'], category))
    ...
    def _construct_worker_command(self):
        worker_command = 'work_queue_worker --cores 64'
    ...
    def _patch_providers(self):
        ...
        if isinstance(self.config.provider, CondorProvider):
            path_to_worker = shutil.which('work_queue_worker')
            self.worker_command = './' + self.worker_command
            self.config.provider.transfer_input_files.append(path_to_worker)
            if self.project_password_file:
                self.config.provider.transfer_input_files.append(self.project_password_file)
    ...
    def initialize_scaling(self):
        ...
        logger.debug("Starting WorkQueueExecutor with provider: %s", self.config.provider)
        self.worker_command = self._construct_worker_command()
        self._patch_providers()

        if hasattr(self.config.provider, 'init_blocks'):
            try:
                self.scale_out(blocks=self.config.provider.init_blocks)
    ...
    def scale_out(self, blocks=1):
        ...
        if self.config.provider:
            for i in range(blocks):
                ...
                internal_block = self.config.provider.submit(self.worker_command, 1)
                # Failed to create block with provider
                if not internal_block:
                    raise(ScalingFailed(self.config.provider.label, "Attempts to create nodes using the provider has failed"))
                else:
    ...
    def scale_in(self, count):
        ...
        if self.config.provider:
            self.config.provider.cancel(kill_ids)
        else:
            logger.error("No execution provider available to scale")
        ...
        try:
            while not self.should_stop.value:
                ...
                try:
                    ...
                    logger.debug("Got task report: {}".format(task_report))
                except queue.Empty:
                    continue

                logger.debug("Sending task result: {}".format(task_report.result))
                self.results_outgoing.send(task_report.result)
...
#                         fu.set_exception(WorkQueueFailure("work queue executor failed to execute the task."))
            ...
            logger.exception("Collector Thread exception")
