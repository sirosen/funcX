...
def unpack_function(function_info):
    try: 
        return unpack_byte_code_function(function_info['buffer'])
    except:
...
def unpack_byte_code_function(function_info):
    from funcx.serialize import FuncXSerializer
    serializer = FuncXSerializer()
    decoded = function_info.decode()
    func, args, kwargs = serializer.unpack_and_deserialize(decoded)
...
def load_function(map_file, function_file):
    ...
    (fn, fn_name, fn_args, fn_kwargs) = unpack_function(function_info)
    ...
    return (user_ns, function_info['task_id'], code, result_name)


def execute_function(namespace, task_id, function_code, result_name):
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            (namespace, task_id, function_code, result_name) = load_function(map_file, function_file)
        except Exception:
            print("There was an error setting up the function for execution.")
            raise

        try:
            result = execute_function(namespace, task_id, function_code, result_name)
        except Exception:
        ...
        result = {'task_id': task_id,
                  'exception': serializer.serialize(RemoteExceptionWrapper(*sys.exc_info()))}
